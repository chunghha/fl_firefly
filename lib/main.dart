...
void main() => runApp(const Firefly());

class Firefly extends StatelessWidget {
  const Firefly({Key? key}) : super(key: key);
...
class CrazyFirefly extends HookWidget {
  const CrazyFirefly({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(
      () {
        ...
        final timer = Timer.periodic(Duration(milliseconds: 200), (time) {
          _ticks++;
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Crazy Firefly'),
      ),
      body: SafeArea(
        child: Column(
